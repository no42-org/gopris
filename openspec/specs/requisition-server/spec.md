# requisition-server Specification

## Purpose

Serve rendered OpenNMS requisition XML over HTTP for Provisiond to import, running the extract → transform pipeline on demand with a TTL cache and serving the last-good rendering when extraction fails.
